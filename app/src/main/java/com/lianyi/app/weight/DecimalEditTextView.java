...
/**
 * @author Lee
 * @name DecimalEditText
 * @describe 自动补位的EditText
...
public class DecimalEditTextView extends AppCompatEditText {
     ...
     * 上限数字 默认1000000
     */
    private double mUpperDecimal = 9999999.99;
    ...
    public DecimalEditTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initView(Context context, AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray attrArray = context.obtainStyledAttributes(attrs, R.styleable.DecimalEditTextView, 0, 0);
            try {
                String symbol = attrArray.getString(R.styleable.DecimalEditTextView_public_decimal_symbol);
                if (symbol != null) {
                    mSymbol = symbol;
                }
                mSymbolSize = attrArray.getDimensionPixelSize(R.styleable.DecimalEditTextView_public_decimal_symbol_size, 0);
                mShowSymbol = attrArray.getBoolean(R.styleable.DecimalEditTextView_public_decimal_show_symbol, false);
                mShowCommas = attrArray.getBoolean(R.styleable.DecimalEditTextView_public_decimal_show_commas, false);
                mUpperDecimal = attrArray.getFloat(R.styleable.DecimalEditTextView_public_decimal_upper, 9999999.99f);
                mDecimalScale = attrArray.getInteger(R.styleable.DecimalEditTextView_public_decimal_scale, 2);
                mDecimalFill = attrArray.getBoolean(R.styleable.DecimalEditTextView_public_decimal_fill_zero, true);
            } finally {
                attrArray.recycle();
            }
        ...
        addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                 ...
                 *                  v1 < v2   return -1;
                 */
                int compareValue = Double.compare(decimalDouble, mUpperDecimal);
                if (compareValue == 1) {
                    if (null != onDecimalUpperListener) {
                        onDecimalUpperListener.onDecimalUpper();
                    }
                    //赋上次的值(格式化)
    ...
    public Double getDoubleDecimalValue() {
        return formatDecimal2Double(TextUtils.isEmpty(getText().toString()) ? "0" : getText().toString());
    }

    public String getStringDecimalValue() {
        return String.valueOf(formatDecimal2Double(TextUtils.isEmpty(getText().toString()) ? "0" : getText().toString()));
